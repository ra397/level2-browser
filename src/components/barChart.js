...
export function barChart(el, { data, yLabel = "", onSelect, labelEvery = 3 }) {
    console.log(data);
    ...
    <div>
      <div class="bars">${data.map(d =>
        `<div class="bar" data-timestamp="${d.timestamp.getTime()}" style="height:calc(5px + ${d.value / max * 100}%)"></div>`).join("")}</div>
